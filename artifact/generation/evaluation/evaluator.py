...
class AccessEvaluator():
    ...
    def __init__(self, true, pred, truesrls = [], predsrls = [], inputs=None, confs = None, true_confs=None, cats = None):
        ...
        ## ---- NEW: per-NLACP Sankey breakdown counters (single/multi-rule x correct/incorrect) ----
        ## "Correct" here is defined per the 'ent_type' scheme (i.e. every true ACR was matched --
        ## exactly or via do_overlap -- and nothing was missed or spuriously over-generated), since
        ## that is the scheme behind the headline "ACR Generation" F1 already reported elsewhere.
        self.sankey_counts = {
            'n_single_correct': 0, 'n_single_incorrect': 0,
            'n_multi_correct': 0, 'n_multi_incorrect': 0,
        }
        ## ---- END NEW ----
    ...
    def evaluate(self):
        ...
        for true_acp, pred_acp, sent, conf, tc, cat in zip(self.true, self.pred, self.inputs, self.confs, self.true_confs, self.cats):
            ...
            is_multi_rule = len(true_acp) > 1
            is_correct = (tmp_results['ent_type']['missed'] == 0
                          and tmp_results['ent_type']['spurious'] == 0)
            if is_multi_rule:
                self.sankey_counts['n_multi_correct' if is_correct else 'n_multi_incorrect'] += 1
            else:
                self.sankey_counts['n_single_correct' if is_correct else 'n_single_incorrect'] += 1
        ...
        n_single = self.sankey_counts['n_single_correct'] + self.sankey_counts['n_single_incorrect']
        n_multi = self.sankey_counts['n_multi_correct'] + self.sankey_counts['n_multi_incorrect']
        self.results['sankey'] = {
            'total': n_single + n_multi,
            'n_single': n_single,
            'n_multi': n_multi,
            'n_single_correct': self.sankey_counts['n_single_correct'],
            'n_single_incorrect': self.sankey_counts['n_single_incorrect'],
            'n_multi_correct': self.sankey_counts['n_multi_correct'],
            'n_multi_incorrect': self.sankey_counts['n_multi_incorrect'],
        }
