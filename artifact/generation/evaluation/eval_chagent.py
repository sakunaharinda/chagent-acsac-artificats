...
@click.option('--result_dir', default='results/sarcp/sensitivity/', 
...
def main(mode, result_dir, k, seed, refine, no_retrieve, no_update, use_pipe):
    
    """Evaluates CHAGent in DSARCP setting."""
    ...
    gen_ckpt = f"../checkpoints/{mode}_act_{seed}/checkpoint/"
    ver_ckpt = "../checkpoints/verification/checkpoint/"
    
    # gen_ckpt = f"../checkpoints/{mode}_act/{seed}/checkpoint/"
    # ver_ckpt = "../checkpoints/verification/checkpoint/"
    ...
    # )
    base_model = AutoModelForCausalLM.from_pretrained(GEN_MODEL)
